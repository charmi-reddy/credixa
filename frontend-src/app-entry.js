...
function popupApi() {
    ...
    btnClosePopup?.addEventListener('click', hide);
...
function renderWallets(wallets) {
    ...
    if (supplierBalance) supplierBalance.textContent = `${wallets.balances?.supplier ?? 0} microAlgos`;
    if (investorBalance) investorBalance.textContent = `${wallets.balances?.investor ?? 0} microAlgos`;
}
...
function invoiceCard(invoice, role) {
    ...
    const investorAction = role === 'investor' && invoice.status === 'CREATED'
        ? `<button class="btn accent fund-btn" data-invoice-id="${invoice.id}">Fund This Invoice</button>`
        : '';
    const buyerAction = role === 'buyer' && invoice.status !== 'PAID'
        ? `<button class="btn primary buyer-fund-btn" data-invoice-id="${invoice.id}">Fund Now</button>`
        : '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (page === 'supplier-login') {
        document.getElementById('btnSupplierLogin')?.addEventListener('click', async () => {
            try {
                ...
                setTimeout(() => { window.location.href = '/supplier/dashboard'; }, 500);
            } catch (err) {
                popup.show('Supplier login failed', err.message, 'Error');
            }
        });

        document.getElementById('btnSupplierRegister')?.addEventListener('click', async () => {
            try {
                const user_id = document.getElementById('supplierUserId').value.trim();
                const password = document.getElementById('supplierPassword').value.trim();
    ...
    if (page === 'investor-login') {
        document.getElementById('btnInvestorLogin')?.addEventListener('click', async () => {
            try {
                ...
                setTimeout(() => { window.location.href = '/investor/dashboard'; }, 500);
            } catch (err) {
                popup.show('Investor login failed', err.message, 'Error');
            }
        });

        document.getElementById('btnInvestorRegister')?.addEventListener('click', async () => {
            try {
                const user_id = document.getElementById('investorUserId').value.trim();
                const password = document.getElementById('investorPassword').value.trim();
    ...
    if (page === 'buyer-login') {
        document.getElementById('btnBuyerLogin')?.addEventListener('click', async () => {
            try {
                ...
                setTimeout(() => { window.location.href = '/buyer/dashboard'; }, 500);
            } catch (err) {
                popup.show('Buyer login failed', err.message, 'Error');
            }
        });

        document.getElementById('btnBuyerRegister')?.addEventListener('click', async () => {
            try {
                const buyer_id = document.getElementById('buyerId').value.trim();
                const password = document.getElementById('buyerPassword').value.trim();
    ...
    if (page === 'supplier-dashboard') {
        const refresh = async () => {
            ...
            document.getElementById('welcomeText').textContent = `Logged in as ${auth?.user_id}. Create invoices for buyers who will pay later.`;
    ...
    if (page === 'buyer-dashboard') {
        const refresh = async () => {
            ...
            await loadDashboard('buyer', invoice => invoice.buyer_id === auth?.buyer_id);
            const welcome = document.getElementById('welcomeText');
            if (welcome) {
                welcome.textContent = `Buyer ${auth?.buyer_id} can see invoices that will eventually be repaid in full.`;
            }
